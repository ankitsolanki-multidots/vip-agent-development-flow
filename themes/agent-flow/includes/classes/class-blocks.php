...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'agent_flow_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( AGENT_FLOW_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( AGENT_FLOW_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function agent_flow_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'agent-flow',
					'title' => __( 'Agent Flow', 'agent-flow' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		$namespace = 'AGENT_FLOW\Blocks\\';

		// Remove namespace from block name.
		if ( false !== strpos( $string, 'agent-flow/' ) ) {
			$string = str_replace( 'agent-flow/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'agent_flow_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'agent_flow_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
